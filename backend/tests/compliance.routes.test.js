...
// Unit tests for backend/routes/compliance.js
...
auth.validateToken = (req, res, next) => { req.user = { id: 'user1', role: 'admin' }; next(); };
checkRole.mockImplementation((roles) => (req, res, next) => next());
nodemailer.createTransport.mockReturnValue({ sendMail: jest.fn((opts, cb) => cb && cb(null, { response: 'ok' })) });

// Helper for file upload
const mockFile = (name = 'file.pdf', mimetype = 'application/pdf') => ({
    fieldname: 'documents',
    originalname: name,
    encoding: '7bit',
    mimetype,
...
describe('Compliance Routes', () => {
    ...
    describe('GET /api/compliance', () => {
        it('should return proposals for admin', async () => {
            Proposal.find.mockReturnValue({ populate: jest.fn().mockReturnThis(), sort: jest.fn().mockResolvedValue([{ id: 1 }]) });
        ...
        it('should filter by complianceStatus', async () => {
            Proposal.find.mockReturnValue({ populate: jest.fn().mockReturnThis(), sort: jest.fn().mockResolvedValue([{ id: 2, complianceStatus: 'pending' }]) });
            const res = await request(app).get('/api/compliance?complianceStatus=pending').set('Authorization', 'Bearer token');
            expect(res.status).toBe(200);
            expect(res.body[0].complianceStatus).toBe('pending');
        });
        it('should filter by partner user', async () => {
            auth.validateToken = (req, res, next) => { req.user = { id: 'partner1', role: 'partner' }; next(); };
            Proposal.find.mockReturnValue({ populate: jest.fn().mockReturnThis(), sort: jest.fn().mockResolvedValue([{ id: 3, submitter: 'partner1' }]) });
            const res = await request(app).get('/api/compliance').set('Authorization', 'Bearer token');
            expect(res.status).toBe(200);
    ...
    describe('POST /api/compliance/:proposalId/documents', () => {
        beforeEach(() => { auth.validateToken = (req, res, next) => { req.user = { id: 'user1', role: 'admin' }; next(); }; });
        ...
        it('should return 403 if partner not owner', async () => {
            auth.validateToken = (req, res, next) => { req.user = { id: 'p2', role: 'partner' }; next(); };
            Proposal.findById.mockResolvedValue({ id: '123', submitter: 'p1', status: 'approved', complianceDocuments: [] });
            const res = await request(app).post('/api/compliance/123/documents').send();
            expect(res.status).toBe(403);
        });
        it('should return 400 if proposal not approved', async () => {
            Proposal.findById.mockResolvedValue({ id: '123', submitter: 'user1', status: 'pending', complianceDocuments: [] });
            const res = await request(app).post('/api/compliance/123/documents').send();
            expect(res.status).toBe(400);
        });
        it('should return 400 if documentTypes length mismatch', async () => {
            Proposal.findById.mockResolvedValue({ id: '123', submitter: 'user1', status: 'approved', complianceDocuments: [] });
        ...
        it('should update complianceDocuments and notify reviewers', async () => {
            const save = jest.fn();
            Proposal.findById.mockResolvedValue({ id: '123', submitter: 'user1', status: 'approved', complianceDocuments: [], save });
        ...
        it('should set complianceStatus to compliant if all required submitted', async () => {
            const save = jest.fn();
            Proposal.findById.mockResolvedValue({ id: '123', submitter: 'user1', status: 'approved', complianceDocuments: [{ name: 'type1', required: true, submitted: true }], save });
            User.find.mockResolvedValue([{ email: 'r@r.com', name: 'R' }]);
            const req = request(app).post('/api/compliance/123/documents');
            req.attach('documents', Buffer.from('file'), 'file1.pdf');
            req.field('documentTypes', JSON.stringify(['type1']));
    ...
    describe('PUT /api/compliance/:proposalId/status', () => {
        ...
        it('should update complianceStatus and add comment', async () => {
            const save = jest.fn();
            Proposal.findById.mockResolvedValue({ id: '123', submitter: 'user1', complianceStatus: 'pending', reviewComments: [], save });
    ...
    describe('GET /api/compliance/overdue', () => {
        it('should return overdue proposals', async () => {
            Proposal.find.mockReturnValue({ populate: jest.fn().mockReturnThis(), sort: jest.fn().mockResolvedValue([{ id: 1, complianceStatus: 'overdue' }]) });
            User.findById.mockResolvedValue({ email: 's@s.com', name: 'S' });
            const res = await request(app).get('/api/compliance/overdue').set('Authorization', 'Bearer token');
            expect(res.status).toBe(200);
    ...
    describe('GET /api/compliance/stats', () => {
        it('should return compliance stats', async () => {
            ...
            const res = await request(app).get('/api/compliance/stats').set('Authorization', 'Bearer token');
            expect(res.status).toBe(200);
