...
describe('Proposal Service', () => {
    ...
    describe('getProposalById', () => {
        it('should get proposal by ID successfully', async () => {
            ...
            const { GridFSBucket } = await import('postgresql');
    ...
    describe('getDebugProposalInfo', () => {
        ...
        it('should get debug info from postgresql for ObjectId', async () => {
            const mockpostgresqlProposal = {
            ...
            Proposal.findById.mockResolvedValueOnce(mockpostgresqlProposal);

            const result = await proposalService.getDebugProposalInfo('507f1f77bcf86cd799439011');

            expect(result.postgresql.found).toBe(true);
            expect(result.postgresql.data.id).toBe('507f1f77bcf86cd799439011');
            expect(result.postgresql.data.title).toBe('Test Proposal');
        });
    ...
    // =============================================
    // postgresql LEGACY FUNCTIONS TESTS
    // =============================================

    describe('createpostgresqlProposal', () => {
        it('should create postgresql proposal successfully', async () => {
            ...
            const mockProposalInstance = {
                save: vi.fn().mockResolvedValueOnce(mockSavedProposal)
            };

            Proposal.mockImplementationOnce(() => mockProposalInstance);

            const result = await proposalService.createpostgresqlProposal(mockUser, mockProposalData, mockFiles);

            expect(mockProposalInstance.save).toHaveBeenCalled();
            expect(result).toEqual(mockSavedProposal);
        });

        it('should send email notification to reviewers', async () => {
            const mockUser = { id: 'user123', name: 'John Doe', email: 'john@example.com' };
            const mockProposalData = { title: 'Test Proposal', status: 'draft' };
            const mockFiles = [];
            ...
            User.find.mockResolvedValueOnce(mockReviewers);
            ...
            await proposalService.createpostgresqlProposal(mockUser, mockProposalData, mockFiles);

            expect(User.find).toHaveBeenCalledWith({ role: 'reviewer' });
            expect(transporter.sendMail).toHaveBeenCalledWith(
                expect.objectContaining({
                    subject: 'New Partnership Proposal Submitted',
                    bcc: ['reviewer1@example.com', 'reviewer2@example.com']
                }),
                expect.any(Function)
            );
        });
    ...
    describe('getpostgresqlProposals', () => {
        it('should get postgresql proposals with filters for student', async () => {
            const mockUser = { id: 'user123', role: 'student' };
            const mockFilters = { status: 'draft', search: 'test' };

            const mockProposals = [
                {
                    _id: '507f1f77bcf86cd799439011',
                    title: 'Test Proposal',
                    submitter: { _id: 'user123', name: 'John Doe', email: 'john@example.com' }
                }
            ];

            Proposal.find.mockReturnValueOnce({
                populate: vi.fn().mockReturnValueOnce({
                    populate: vi.fn().mockReturnValueOnce({
                        sort: vi.fn().mockResolvedValueOnce(mockProposals)
                    })
                })
            });

            const result = await proposalService.getpostgresqlProposals(mockUser, mockFilters);
        ...
        it('should get postgresql proposals without user filter for admin', async () => {
            const mockUser = { id: 'admin123', role: 'head_admin' };
            ...
            await proposalService.getpostgresqlProposals(mockUser, mockFilters);
            ...
            expect(Proposal.find).not.toHaveBeenCalledWith(
                expect.objectContaining({
                    submitter: expect.anything()
                })
            );
        });
    });
    ...
    describe('Error Handling', () => {
        ...
        it('should handle file metadata retrieval errors', async () => {
            query.mockResolvedValueOnce(mockQueryResult);
            getDb.mockRejectedValueOnce(new Error('postgresql connection failed'));
