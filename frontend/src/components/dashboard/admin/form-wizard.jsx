...
export function FormWizard({ steps, onComplete, className, initialStep = 0 }) {
  ...
  return (
    <div className={cn("space-y-8", className)}>
        ...
        <nav aria-label="Progress">
          <ol role="list" className="flex items-center">
            {steps.map((step, index) => (
              <li key={step.title} className={cn(index !== steps.length - 1 ? "pr-8 sm:pr-20" : "", "relative")}>
                {index < currentStep ? (
                  <>
      ...
      <div className="sm:hidden">
        ...
        <h3 className="text-lg font-medium text-cedo-blue">{steps[currentStep].title}</h3>
      </div>
