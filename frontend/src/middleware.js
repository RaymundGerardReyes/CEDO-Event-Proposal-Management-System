...
async function verifyAuthToken(token) {
  if (!token || !JWT_SECRET) { // Check if JWT_SECRET is available
    return null;
  }
  try {
    ...
    console.error("Middleware: JWT Verification Error:", err.message, "(Token might be expired, malformed, or secret mismatch)");
    return null;
  }
}
...
export async function middleware(request) {
  ...
  if (publicPaths.includes(pathname) && (pathname === "/sign-in" || pathname === "/sign-up")) {
    ...
    if (userData.dashboard) { // Prefer dashboard path from user object
  ...
  if (pathname.startsWith("/admin-dashboard") &&
    ...
    console.log(`Middleware: Access Denied to "${pathname}". User role "${userData.role}" not HEAD_ADMIN or MANAGER. Redirecting.`);
    // Redirect to their default dashboard or an "access denied" page
    const fallbackDashboard = (userData.role === ROLES.STUDENT || userData.role === ROLES.PARTNER) ? "/student-dashboard" : "/";
    return NextResponse.redirect(new URL(fallbackDashboard, origin));
  }

  if (pathname.startsWith("/student-dashboard") &&
    userData.role !== ROLES.STUDENT &&
    userData.role !== ROLES.PARTNER) { // Example: if only students/partners can access student dashboard
    console.log(`Middleware: Access Denied to "${pathname}". User role "${userData.role}" not STUDENT or PARTNER. Redirecting.`);
    const fallbackDashboard = (userData.role === ROLES.HEAD_ADMIN || userData.role === ROLES.MANAGER) ? "/admin-dashboard" : "/";
    return NextResponse.redirect(new URL(fallbackDashboard, origin));
  }
