...
import { Section4_CommunityEvent } from './Section4_CommunityEvent.jsx';

export default function CommunityEventPage({ params }) {
    ...
    useEffect(() => {
        if (draft) {
            const communityEventData = draft.form_data?.communityEvent ||
                draft.payload?.communityEvent ||
    ...
    useDebouncedCallback(() => {
        if (!loading && draft) {
            patch({ section: 'communityEvent', payload: { communityEvent: formData } });
        }
