...
export const useWhitelist = (authUser) => {
    ...
    const loadUsers = useCallback(async (showRefreshLoader = false) => {
        try {
            ...
            const transformedUsers = response.map((user, index) => ({
                ...
                avatarUrl: user.avatarUrl || user.profile_picture_url || null,
            ...
            } else if (error.message.includes('connect') || error.code === 'NETWORK_ERROR') {
                errorMessage = 'Unable to connect to the server. Please check your internet connection and ensure the backend is running.';
            } else if (error.message.includes('Access denied') || error.response?.status === 403) {
                errorMessage = 'Access denied. You do not have permission to view users. Please ensure you are logged in with admin privileges.';
            } else if (error.message.includes('Authentication expired') || error.response?.status === 401) {
                errorMessage = 'Authentication expired. Please sign in again.';
            } else if (error.response?.status === 500) {
                errorMessage = 'Server error. Please try again later.';
            } else {
                errorMessage = error.message || 'Failed to load whitelisted users.';
            }
    ...
    const validateForm = useCallback(() => {
        ...
        // Special validation for manager and head_admin roles
        if ((newUser.role === "manager" || newUser.role === "head_admin") && !generatedPassword) {
            errors.role = `Please generate a password for the ${newUser.role} account`;
        }

        setFormErrors(errors);
        return Object.keys(errors).length === 0;
    }, [newUser, users, generatedPassword]);
    ...
    /**
     * Handle role change and auto-generate password for managers and head admins
     */
    const handleRoleChange = useCallback((role) => {
        setNewUser(prev => ({ ...prev, role }));

        if (role === "manager" || role === "head_admin") {
            // Auto-generate password for manager and head_admin roles
            const autoPassword = generateSecurePassword();
            setGeneratedPassword(autoPassword);
            setPasswordCopied(false);
            setShowPassword(false); // Reset password visibility
            console.log(`🔐 [useWhitelist] Auto-generated password for ${role} role:`, autoPassword);
        } else {
            // Clear password for other roles
    ...
    const handleAddUser = useCallback(async (e) => {
        ...
        try {
            ...
            const userData = {
                ...
                ...((newUser.role === "manager" || newUser.role === "head_admin") && generatedPassword && {
                    password: generatedPassword // Password required for manager and head_admin roles
                })
            };
            ...
            const newWhitelistedUser = {
                ...
                organization: response.user?.organization || response.organization || '',
                addedAt: response.user?.created_at || response.created_at,
                addedBy: authUser.email,
                is_approved: response.user?.is_approved || response.is_approved
            };
            ...
            // Show appropriate success message based on role and password generation
            const successMessage = (newUser.role === "manager" || newUser.role === "head_admin") && response.user?.generated_password
                ? `${newUser.name} (${newUser.email}) has been added as a ${newUser.role} with generated password: ${response.user.generated_password}`
                : (newUser.role === "manager" || newUser.role === "head_admin")
                    ? `${newUser.name} (${newUser.email}) has been added as a ${newUser.role} with the provided password.`
            ...
            toastRef.current({
                ...
                variant: "default",
                duration: 6000, // Longer duration for manager passwords
            });

            console.log('✅ [useWhitelist] Successfully added user:', newUser.name);
            if (response.user?.generated_password) {
                console.log('🔐 [useWhitelist] Generated password for manager:', response.user.generated_password);
            }
    ...
    const saveEdit = useCallback(async (userId, field, newValue) => {
        ...
        try {
            ...
            setUsers(prev => prev.map(user =>
                user.id === userId
                    ? { ...user, [field]: newValue, updated_at: response.updated_at }
                    : user
            ));
