...
const OverviewTab = ({ proposal }) => {
    ...
    const fetchMongoFiles = async () => {
        ...
        try {
            ...
            if (!token) {
                console.warn('⚠️ No authentication token found for admin file fetch');
            ...
            if (response.ok) {
                ...
                if (data.success && data.files && getSafeFileCount(data.files) > 0) {
                    ...
                    console.log(`✅ Found ${getSafeFileCount(data.files)} files in MongoDB for proposal ${proposal.id}`);

                    toast({
                        title: "Files Loaded Successfully",
                        description: `Found ${getSafeFileCount(data.files)} files from MongoDB (${data.source})`,
                        variant: "default"
                    });
                } else {
                    ...
                    if (proposal.files && getSafeFileCount(proposal.files) > 0) {
                        toast({
                            title: "Files Updated",
                        ...
                        <div className="flex items-center gap-2">
                            {filesFetched && (
                                <span className="text-xs text-gray-500">
                                    {getSafeFileCount(mongoFiles) > 0 ?
                                        `${getSafeFileCount(mongoFiles)} files from MongoDB` :
                                        'Using cached data'
                                    }
                                </span>
                            )}
                            ...
                            {process.env.NODE_ENV === 'development' && (
                                <span className="text-xs text-blue-500 bg-blue-50 px-2 py-1 rounded">
                                    ...
                                    Count: {getSafeFileCount(mongoFiles)} |
                                    ID: {proposal.id}
                                </span>
                            )}
                            <Button
