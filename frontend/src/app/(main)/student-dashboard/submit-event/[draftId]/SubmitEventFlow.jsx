...
const SubmitEventFlow = () => {
    ...
    const getDefaultFormData = () => ({
        ...
        reportStatus: "draft",
        validationErrors: {},
        // ✅ PROPOSAL ID FIELDS: Include in defaults
        id: null,
        proposalId: null,
        ...
        if (!hasInitialized) {
            ...
            if (initialStateValue !== STATUS.OVERVIEW && validation.allowed) {
                ...
                if (initialStateValue === STATUS.EVENT_TYPE_SELECTION) {
                    ...
                    // ✂️ REPLACED TIMEOUT CASCADE WITH SINGLE ATOMIC TRANSITION
                    send({ type: "START_PROPOSAL" });
                    send({
                        type: "SELECT_EVENT_TYPE",
                        data: {
    ...
    const handleSection3Next = useCallback((targetSectionOrBypass) => {
        console.log(`🚀 handleSection3Next called with:`, targetSectionOrBypass);
        ...
        if (!isBypassing && !validateAndSetErrors('schoolEvent')) {
            console.log('❌ Section 3 validation failed. Halting navigation.');
            return;
        }

        if (isBypassing && targetSection) {
            console.log(`🔀 BYPASS: Jumping from Section 3 to "${targetSection}"`);
            // 🔧 CRITICAL FIX: Manually update formData BEFORE dispatching to the state machine.
            ...
            console.log('✅ Section 3 validation passed. Proceeding to next section.');
            send({ type: 'NEXT' });
        }
